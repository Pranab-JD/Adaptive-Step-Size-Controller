...
class Diffusion_Advection_1D_Adaptive_h:
    ...
    def initialize_parameters(self):
        ...
        self.dt = 1.25 * min(self.adv_cfl, self.diff_cfl)   # N * CFL condition
    ...
    def Error(self, u, u_sol, dt):
        ...
        u_ref_1, u, its_ref_1 = self.Solution(self.u, dt/2)
        u_ref, u, its_ref_2 = self.Solution(u_ref_1, dt/2)
        error = np.mean(abs(u_sol - u_ref))

        return u_ref, error, its_ref_1 + its_ref_2
    ...
    def run(self):
        ...
        path = os.path.expanduser("~/PrJD/Diffusion_Advection/1D/C- 100/N_" + str(n_val) + "/Non Penalized/tol " + str(emax))
        path_sim = os.path.expanduser("~/PrJD/Diffusion_Advection/1D/C- 100/N_" + str(n_val))
        ...
        while (time < self.tmax):
            ...
            if counter < 2:
                ...
                u_ref, error, num_mv_error = self.Error(self.u, u_sol, dt)
                ...
                cost_trad = num_mv_sol + num_mv_error
                cost_cont = 0

                dt_temp.append(dt)

            ### dt for final time step
            elif time + dt >= self.tmax:

                dt_final = self.tmax - time

                ### Solution
                u_sol, u, num_mv_final = self.Solution(self.u, dt_final)
                u_ref, error, num_mv_final_err = self.Error(self.u, u_sol, dt_final)
                ...
                if error > self.error_tol:
                    ...
                    u_sol, u_ref, dt_inp, dt_used, dt_trad, num_mv_trad = Richardson_Extrapolation_2(self.A_adv, 2, self.A_dif, 1, self.Solution, Method_order, \
                                                                                                    u_sol, u, dt_final, self.error_tol)

                    ## its_ref_1 added in num_mv_trad
                    cost_trad = num_mv_final + num_mv_final_err


                else:
                    cost_cont = num_mv_final
                    dt = dt_final
