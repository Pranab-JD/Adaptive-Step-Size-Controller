...
Description:
    ...
    du/dt = d^2(u^2)/dx^2 + eta_x * du/dx
          + d^2(u^2)/dy^2 + eta_y * du/dy (2D) 
...
class Porous_Medium_1D(Cost_Controller_1D):
...
class Porous_Medium_2D(Cost_Controller_2D):
    
    def __init__(self, N_x, N_y, tmax, eta_x, eta_y, error_tol):
        super().__init__(N_x, N_y, tmax, eta_x, eta_y, error_tol)
        self.x_1 = 0.25                         # Position of Heaviside function 1X
        self.x_2 = 0.6                          # Position of Heaviside function 2X
        self.y_1 = 0.25                         # Position of Heaviside function 1Y
        self.y_2 = 0.6                          # Position of Heaviside function 2Y
        self.initialize_U()
        self.eigen_linear_operators()
    ...
    def initialize_U(self):
        u0 = 1 + np.heaviside(self.x_1 - self.X, self.x_1) + np.heaviside(self.X - self.x_2, self.x_2) \
               + np.heaviside(self.y_1 - self.Y, self.y_1) + np.heaviside(self.Y - self.y_2, self.y_2)
        self.u = u0.copy()
